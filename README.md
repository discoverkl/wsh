...
Sessions are cleaned up 90 minutes after the last writer disconnects — long enough that a coding agent left running in a tab finishes its task even if you close the browser. Click the **pin button** (owner only) to keep a session alive indefinitely — useful for long-running processes you want to check back on. Unpinning restarts the timer. Pinned state is in-memory only and does not survive a server restart.

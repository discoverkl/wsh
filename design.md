...
- `/api/apps`: non-allowed callers see only public apps — web *and* pty (Skills and private apps hidden). Those apps are then reported **unhidden**, whatever `hidden` says. The flag arranges the owner's catalog and a stranger is not looking at that page; what decides who may reach an app is `access`, and the filter enforcing it has already run. Without the exemption an image that ships its apps hidden would serve a stranger a catalog whose entire content is the "All apps" expander — and with nothing hidden left in the payload the client draws no expander at all, so they cannot collapse their own page to nothing either. `_raw` is stripped of `hidden` for the same reason: the card's config popover lists every key it finds.

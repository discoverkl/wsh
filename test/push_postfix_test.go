...
func TestPushPostfixShippedScriptNormalizesEndpoints(t *testing.T) {
  ...
  - name: mine
    open_ai:
      ...
      base_url: https://aidp-i18ntt-sg.tiktok-row.net/api/modelhub/online
`
	...
	if !strings.Contains(string(got), "aidp-i18ntt-sg.byteintl.net") {
		t.Errorf("endpoint not normalized to prod:\n%s", got)
	}
	if strings.Contains(string(got), "tiktok-row.net") {
